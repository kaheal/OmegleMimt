...
var Omegle = function () {
  ...
  var evalCaptcha = function (pchallengeLink) {
    ...
    var url =
      'http://www.google.com/recaptcha/api/challenge?k=' + pchallengeLink;
  ...
  this.send = function (msg) {
    if (gotID && isConnected) {
      ...
      _this.emit(
        'omerror',
        'send(): Not connected to ' +
          (gotID ? 'a stranger yet.' : 'the server.')
      );
    }
  };
  this.startTyping = function () {
    if (gotID && isConnected) {
      ...
      getResponse('/typing', data, function (body, error) {
        if (body) {
          if (body != 'win')
            _this.emit(
              'omerror',
              "startTyping(): Couldn't send the typing event. Response from server: " +
                body
            );
      ...
      _this.emit(
        'omerror',
        "startTyping(): Couldn't send the typing event. Not connected to " +
          (gotID ? 'a stranger yet.' : 'the server.')
      );
  };
  this.stopTyping = function () {
    if (gotID && isConnected) {
      ...
      getResponse('/stoppedtyping', data, function (body, error) {
        if (body) {
          if (body != 'win')
            _this.emit(
              'omerror',
              "stopTyping(): Couldn't send the stoppedtyping event. Response from server: " +
                body
            );
      ...
      _this.emit(
        'omerror',
        "stopTyping(): Couldn't send the stoppedtyping event. Not connected to " +
          (gotID ? 'a stranger yet.' : 'the server.')
      );
  };
      ...
      getResponse('/stoplookingforcommonlikes', data, function (body, error) {
        if (body) {
          if (body != 'win')
            _this.emit(
              'omerror',
              'stopLookingForCommonLikes(): Something went wrong. Response from server: ' +
                body
            );
        } else _this.emit('omerror', error);
  ...
  this.disconnect = function () {
    if (gotID && isConnected) {
      ...
      getResponse('/disconnect', data, function (body, error) {
        if (body) {
          if (body != 'win')
            _this.emit(
              'omerror',
              "disconnect(): Couldn't send the disconnect event. Response from server: " +
                body
            );
          reset();
      ...
      _this.emit(
        'omerror',
        "disconnect(): Couldn't send the disconnect event. Not connected to " +
          (gotID ? 'a stranger yet.' : 'the server.')
      );
  };
  ...
  this.solveReCAPTCHA = function (answer) {
    if (gotID && challenge) {
      ...
      _this.emit(
        'omerror',
        "solveReCAPTCHA(): Not connected to the server or there's no ReCAPTCHA."
      );
  };
  var reset = function () {
    isConnected = false;
